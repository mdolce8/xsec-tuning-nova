...
# to run: 
#   $ cp hadd_nd_W_quantiles_preds.sh </path/to/grid/files>
#   $ source /path/to/grid/output/files/hadd_nd_W_quantiles_preds.sh <var>"
...
do
  ...
  do
      hadd_cafana $OUTDIR/pred_interp_${quantile}_nxp_xsec24_nd_${beam}_trueW.root pred_interp_${quantile}_nxp_xsec24_nd_${beam}_trueW.*of*.root
  done
