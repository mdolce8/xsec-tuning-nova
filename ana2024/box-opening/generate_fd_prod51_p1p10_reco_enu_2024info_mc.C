/*
 * generate_fd_prod5.1_p1p10_reco_enu_2024info_mc.C:
...
// =====================================================================================================
void generate_fd_prod51_p1p10_reco_enu_2024info_mc(const std::string& beam,        // fhc or rhc
...
// =====================================================================================================
{
  ...
  for (unsigned int quantileIdx = 0; quantileIdx < cutQuantiles.size(); quantileIdx++) {
    spectrumMap.try_emplace(Form("pred_interp_Q%d", quantileIdx + 1),
                        new Spectrum(loader, kNumuCCOptimisedAxis2024, kNumu2024FD && cutQuantiles[quantileIdx], kNoShift, kXSecCVWgt2024 * kPPFXFluxCVWgt));
  }

  spectrumMap.try_emplace("pred_interp_Q5", new Spectrum(loader, kNumuCCOptimisedAxis2024, kNumu2024FD, kNoShift, kXSecCVWgt2024 * kPPFXFluxCVWgt));
  ...
  for (const auto &specPair : spectrumMap){
    std::string fileName = Form("spectra_fd_prod5.1_p1p10_reco_enu_2024info_mc_%s_numu_Q%i.root",  beam.c_str(), quantileCount);
